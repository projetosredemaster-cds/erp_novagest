...
**Frontend e backend estão integrados.** `RankingPage.jsx` busca `diretores` (cada um com suas `redes[]` aninhadas), `categorias` e `entradas` via HTTP (helper em `frontend/src/modulos/ranking/rankingApi.js`) e persiste valores com `POST /api/ranking/entradas` (upsert) no `onBlur` de cada input — não há mais estado fictício inicial. O contrato exato de cada rota (formato de request/response, status codes) está documentado em `CONTRATO-RANKING-API.md` na raiz do repo; consulte-o antes de mudar o formato de qualquer endpoint do Ranking. A URL base da API vem da variável de ambiente Vite `VITE_API_URL` (ver `frontend/.env.example`) — nunca hardcode a URL no componente.
...
- Não existe endpoint de exclusão para `Entradas` (só upsert) — na prática isso significa que uma rede que já recebeu algum lançamento nunca poderá ser deletada fisicamente, só desativada.
