...
async function enviarEmailRecuperacaoSenha({ destinatario, link }) {
  const apiKey = process.env.BREVO_API_KEY;
  const remetente = process.env.EMAIL_REMETENTE;
  const remetenteNome = process.env.EMAIL_REMETENTE_NOME;

  if (!apiKey || !remetente || !remetenteNome) {
    throw new Error(
      'Variáveis de ambiente de e-mail (BREVO_API_KEY, EMAIL_REMETENTE, EMAIL_REMETENTE_NOME) não estão configuradas.'
    );
  }

  const texto = `Recebemos um pedido de recuperação de senha para sua conta no *erp_Novagest*.\n\nPara redefinir sua senha, acesse o link abaixo (válido por 1 hora):\n${link}\n\nSe você não solicitou essa recuperação, ignore este e-mail.`;

  const payload = {
    sender: { email: remetente, name: remetenteNome },
    to: [{ email: destinatario }],
    subject: 'Recuperação de senha — erp_Novagest',
    htmlContent: formatarTextoParaHtml(texto),
  };
...
module.exports = { enviarRelatorioEmail, enviarEmailRecuperacaoSenha };
